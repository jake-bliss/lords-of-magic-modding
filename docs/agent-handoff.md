...
| Stage 1 asset layer | 9,804 core archive members classified; 1,377 PBMs decoded; 1,800 IMPs expanded; 26 tilesets and every loose map bounded; map-object records parsed across six record layouts in **every** map of both attested profiles — **353 maps / 13,105 records** in the stock `map/` tree (Steam, Development, 3.02) and **365 / 21,117** in GS5R3's, which is the profile `docs/map-format.md` quotes. **Reimport and lossless repack landed 2026-09-18 (#70)** — Observed in the corpus and now corpus-gated tests rather than a manual run: every PBM member of `pic.mpq` re-encodes pixel-lossless (1,045 stock, 1,377 GS5R3) and 41,373 of 41,373 IMP frames do, with 35,840 of 35,840 eligible whole-file rewrites byte-identical | IMP **frame timing**, 10 metadata mismatches + 4 catalog orphans, what the record layouts' constant tail words mean, batch asset conversion, portable packaging (IMP **placement** is solved — see [hotspots](hotspots.md); **shadow blend and palette channel order were both solved 2026-09-17** and are no longer gaps) |
...
**Seven probes exist**, registered in `PROBES` in `tools/engine_probe.py` — `ladder`, `elevation`, `mapsize`, `flatground`, `maptag`, `mapload`, `terrainrings`. (This sentence read "Five" until 2026-09-19; `mapload` and `terrainrings` were added without it being updated. Count the registry, not the prose.) `LOM_PROBE=ladder` (the default) is the four-rung compositing diagnostic that
...
`LOM_PROBE=terrainrings` are documented in their own run sheets. `LOM_PROBE=unitanchor` addresses
the one limit `hotspots.md` states about its own central result: record 0 was confirmed as the draw
placement by injecting a unit IMP through the *terrain sprite* path, not the *unit* path, so a
unit-specific constant offset in the anchor was never ruled out. It places a real recruited unit
(`add_unit_to_location`, copied verbatim from a shipped call site) alongside a terrain-sprite
control on a shared cell. **It ran 2026-09-19 and the residual is zero in x and y**, for the unit
body and independently for the player flag — *Inferred*, conditional on two frame identifications.
It has since been **rebuilt** to three cells and a different subject unit, because that run carried
one facing, one unit type, and a frame that could not settle the mirror sign. ⚠️ Its first reading
was wrong for a reason worth carrying forward: the sheet *named* the drawn art instead of deriving
it, and named the wrong file — `gs\imps.gs`'s `unit_zoom_letter` sends the world map to
`units\imp\<name>b.imp`, not `...a.imp`, and a world-map army is a composite of body, flag, bars and
group number rather than one sprite. See [the run sheet](unit-anchor-run-sheet.md).
...
- **`screencapture` writes G, R, B, not the BMP-standard B, G, R** (**corrected 2026-09-23**; this
  said R, G, B from 2026-09-17, which itself swapped red and green — see [research
  log](research-log.md#2026-09-23--imp-palettes-are-bgr-after-all-the-capture-reader-swapped-red-and-green))
  — and its `bfOffBits` says 14 against a real pixel offset of 54. Equality differencing is blind to
  the first, so it went unnoticed for several runs; any colour conclusion drawn through a standard
  reader is wrong. Use `tools/probe_captures.py`.
...
- **Reaching a real game needs a human** — so menu navigation has to be asked for. The user has
  been willing; make each run count.

  **The stated reason is UNVERIFIED, and was probably wrong.** This file asserted that "macOS blocks
  synthetic input to Wine". What was actually observed, in `map-format.md` and
  `native-asset-stage.md`, is that *"macOS accessibility controls prevented reliable programmatic
  interaction"* — a permission symptom, recorded here as a property of Wine. On 2026-09-19 the first
  AppleScript step was tried from this project's own shell and returned **`-1743: Not authorized to
  send Apple events to System Events`**, i.e. it failed at the TCC gate without ever reaching the
  Wine window. Whether a Wine window accepts posted events is still **unknown** — the point is that
  nothing here has ever tested it. Do not repeat the claim as settled; if automation is wanted, grant
  Accessibility and Screen Recording first and then measure, with a native-app keypress as the
  instrument check before any conclusion about Wine.
...
- **Reaching a real game needs a human** — menu navigation has to be requested. Make each run count.
  The reason long recorded here, "macOS blocks synthetic input to Wine", is **unverified**; see the
  note above.
...
- IMP compositing is keyed by palette **index**, not colour: slot 0 is transparency and slot 1 is the shadow, and **slot 1 draws the background at half brightness** (measured 2026-09-17). The RGB in those slots really is ignored — rewriting slot 1 to magenta rendered identically to the untouched copy. Palette entries are stored **blue, green, red, pad** (plain BGRA; **corrected 2026-09-23** — this said blue, red, green, pad from 2026-09-17, which was itself the swapped reading; see [research log](research-log.md#2026-09-23--imp-palettes-are-bgr-after-all-the-capture-reader-swapped-red-and-green)).
