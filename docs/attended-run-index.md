...
| **D2** the two declared caps | ✅ **DONE 2026-09-21**, with its claims scoped. `maxauratypes` raised 70->100 and two auras registered at **70/71** (*Observed in gameplay*). A unit type at **199** registered and an army was placed at the subject cell, `armyat` confirming the location — that the army *is* the type at 199 is **Derived** from that plus `lastunittype`, not read back. One definition past capacity returned **-1**, left the count at 200 and did not crash the session. ⚠️ `maxunittypes` itself was **never raised**. Took three attempts, both failures mine ([run sheet](unit-cap-run-sheet.md)). 🔴 **Offline follow-up, same day (*Observed in a local binary*):** the static audit says a raise is safe, needs `/unittypedict N dict` moved in lockstep, and hits a **silent ceiling at 1000** imposed by a fixed 1000-dword stack histogram at `0x0052BE20` — see [the ceiling audit](new-units.md#-the-real-ceiling-is-1000-unit-types--and-it-is-silent) |
| **D3** the portrait panel | ✅ **DONE 2026-09-21** — a `pic.mpq`-only build (no script, no probe, no hotkey) settled every branch of the panel's portrait selection *in the engine*, with a control. In one army the two slots **disagreed** — faith emblem bottom-left, replaced portrait in the pop-out — which no single-branch model produces. First attempt, after this question had cost three sittings when it carried a probe ([run sheet](portrait-member-run-sheet.md)) |
| **D4** the added member | ✅ **DONE 2026-09-21** — ⭐ **the engine reads an ADDED `pic.mpq` member.** The portrait table was pointed at `zzadd001.lbm`, a name that never existed in the game; the member was added and the engine drew it, with a live-archive control. Closes open question 2. 🔴 The same sitting **refuted** D3's central claim: GS5R3 resolves portraits through a unit-type table, and D3's six green rungs were all measured on the one unit for which both models name the same file ([run sheet](portrait-member-run-sheet.md)) |
