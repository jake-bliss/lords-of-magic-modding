...
| 9 | Does the engine's portrait lookup match `get_unit_portrait_name`? | **open** — a type with `/faith EARTH` `/code WMT` drew a **fallback** portrait and roster figure while its overworld sprite was correct (*Observed in gameplay*, 2026-09-21). `portrait\EAWMTP00.LBM` is **absent from GS5R3's own `pic.mpq`**, probed by name hash against the archive the run used (`PyELEP00` and `ORINFP00` are present in the same probe), which is consistent with [the `(faith, code)` rule](#4-portrait--one-member) but does not prove the engine uses it. Adding that member would settle it **and** close question 2. The **roster figure** is a third channel whose source is untraced. |
