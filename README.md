...
Concretely, today you can **inspect, decode, render, export, measure, validate and build**. A mod source tree goes through static validation and a deterministic repack to a verified archive with a change report ([build pipeline](docs/build-pipeline.md)). The last step has now happened, for **every archive the pipeline targets**. **Observed in gameplay:** `Lords of Magic Development.app` exists as a cloned profile, and across four attended sittings the engine has loaded archives this pipeline built and rendered or played the change for a human to read — a `gs.mpq` member (2026-09-16, again as the Phase 4 slice `units\orinf.gs` on 2026-09-18, and a size-changing token flip on 2026-09-19), a `pic.mpq` image (2026-09-18, and again on 2026-09-20 through the full ByteRun1 encoder at a length that *shrank*), `sndfx.mpq` and `special.mpq` (2026-09-19, the STORED storage class, two members each in one build), and `imp.mpq` (2026-09-20 — our IMP pixel encoder's output rendered, and a member the archive never had added to it).

**What those runs do not establish, stated as narrowly as the evidence allows.** Both MPQ storage classes are now covered, size-changing edits are accepted in `gs.mpq` and `pic.mpq`, two members in one build are accepted, and an **added** member is accepted in `imp.mpq`. What remains open is narrower and archive-specific: whether the engine can **read** an added member (nothing in the game asks for one, so only the archive's own hash table was shown to resolve it); a size-changing edit in `imp.mpq` specifically, since all three of its rungs were length-preserving by construction; and a `TINY` thumbnail regenerated rather than dropped. Nothing here says a *large* mod loads — the biggest build so far changed four members across two archives. The `engine-acceptance` blocks in the [roadmap](docs/roadmap.md) are the authority for this and are rendered from `tools/engine_acceptance.py` rather than typed; read them rather than this summary. There is still no native game — Phase 3 of the roadmap is substantially complete, **Phase 4 completed 2026-09-18**, and the remaining asset-pipeline work sits in Phase 5.
