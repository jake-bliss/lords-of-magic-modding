...
Concretely, today you can **inspect, decode, render, export, measure, validate and build**. A mod source tree goes through static validation and a deterministic repack to a verified archive with a change report ([build pipeline](docs/build-pipeline.md)). The last step has now happened. **Observed in gameplay:** `Lords of Magic Development.app` exists as a cloned profile, and the engine has loaded two archives this pipeline built and rendered the change for a human to read — a `gs.mpq` member (attended 2026-09-16, and again as the Phase 4 slice `units\orinf.gs` on 2026-09-18) and a `pic.mpq` image (2026-09-18).

**What those runs do not establish, stated as narrowly as the evidence allows.** Each was **one** member of **one** archive, **replaced rather than added**, with a **length-preserving** edit, in the single storage class `0x80010100` (EXISTS | ENCRYPTED | IMPLODE). The engine has never been shown a size-changing edit, a member *added* to an archive, two members changed in one build, any other flag combination, or `imp.mpq`, `sndfx.mpq` and `special.mpq` at all. Nothing here says a *large* mod loads — only that a correct one-member rewrite does. The `engine-acceptance` blocks in the [roadmap](docs/roadmap.md) are the authority for this and are rendered from `tools/engine_acceptance.py` rather than typed; read them rather than this summary. There is still no native game — Phase 3 of the roadmap is substantially complete, **Phase 4 completed 2026-09-18**, and the remaining asset-pipeline work sits in Phase 5.
